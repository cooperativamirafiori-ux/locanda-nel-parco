...
function SeatsBadge({ value, label }: { value: number | null; label: string }) {
  if (value === null) return null;
  return (
    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
      value === 0 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
    }`}>
      {label}: {value === 0 ? 'chiuso' : `${value} posti`}
    </span>
  );
}
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Sovrascrive la capienza standard per pranzo e/o cena in una data specifica.
        Metti <strong>0</strong> per chiudere un servizio.
      </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                max={500}
                value={pranzo}
                onChange={e => setPranzo(e.target.value)}
                placeholder="default"
                className="field"
              />
